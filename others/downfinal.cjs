...
// 计算文件的 MD5 哈希值
function getFileMD5Hash(data) {
    const hash = crypto.createHash('md5');  // 使用 md5 哈希算法
    hash.update(data);
    return hash.digest('hex');
}
...
async function downloadPackage(url, outputDir) {
    ...
    try {
        ...
        const fileHash = getFileMD5Hash(response.data);  // 计算文件的哈希值
        const fileNameWithHash = `${path.basename(url, path.extname(url))}-${fileHash}${path.extname(url)}`;  // 新的文件名包括哈希值
        const outputPathWithHash = path.join(outputDir, fileNameWithHash);

        fs.writeFileSync(outputPathWithHash, response.data);  // 保存文件
        console.log(`Downloaded ${fileNameWithHash} to ${outputPathWithHash}`);
        return true;
    } catch (error) {
